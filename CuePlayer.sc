...
CuePlayer { 
  ...
  *new { arg name;
  ...
  initResources {
    ...
    cuePatternFunction = {arg offset1 = 0; Pseq(list: (1,2..cueList.size), repeats: inf, offset: offset1).asStream;};
  ...
  createAdditionalElements {
    ...
    .states_([["Next Cue / FootSwitch", Color.white, Color(0.2, 0.6, 0.1)]]).action_(
      { 
        var cueNum; cueNum = cuePattern.next.postln; cueList.at(cueNum).value;
